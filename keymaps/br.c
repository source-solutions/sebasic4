...
int main()
{
	...
	// row 1
	MAP(PC__1,SE_APOSTRO);
	MAP(MD_SHIFT|PC__1,SE_QUOTE);
	MAP(PC_1,SE_1);
	...
	MAP(PC_6,SE_6);
	MAP(MD_SHIFT|PC_6,SE_PAROPEN);					// ALT:( = ¨
	MAP(PC_7,SE_7);
	...
	MAP(PC_P1,SE_4);						// ALT:4 = ´
	MAP(MD_SHIFT|PC_P1,SE_POUND);
	MAP(PC_P2,SE_BRAOPEN);
	MAP(MD_SHIFT|PC_P2,SE_CUROPEN);
	...
	MAP(PC_L1,SE_STAR);						// ALT:u = º
	MAP(MD_SHIFT|PC_L1,SE_COLON);			// ALT:c = ª
	MAP(PC_L2,SE_TILDE);
	MAP(MD_SHIFT|PC_L2,SE_CARET);
	MAP(PC_L3,SE_BRACLOS);
	MAP(MD_SHIFT|PC_L3,SE_CURCLOS);

	// row 4
	MAP(PC__Z,SE_BACKSLA);
	MAP(MD_SHIFT|PC__Z,SE_PIPE);
	MAP(PC_Z,SE_Z);
	...
	MAP(PC_M1,SE_COMMA);
	MAP(MD_SHIFT|PC_M1,SE_LESS);
	MAP(PC_M2,SE_DOT);
	MAP(MD_SHIFT|PC_M2,SE_GREATER);
	MAP(PC_M3,SE_SEMICOL);
	MAP(MD_SHIFT|PC_M3,SE_COLON);
	MAP(PC_M4,SE_SLASH);
	MAP(MD_SHIFT|PC_M4,SE_QUEST);
	MAPANY(PC_UP,SE_UP);
	...
	SAVEMAPBIN("../chloehd/SYSTEM/KEYBOARD.S/BR.KB");
