...
.ifdef ROM0
	...
	jp		bankwipe				; clear full RAM in case of a hard reset

	defs	1, 255					; 1 spare bytes

switch:
	out		(c), a					; select ROM 1
.endif

.ifdef ROM1
	di								; switch off interrupt
	xor		a						; A = 0
	jp		ram_check				; clear normal RAM in case of a hard reset
...
_error_1:
	ld		hl, (chadd)				; copy address of current character
	ld		(xptr), hl				; to error pointer
	jr		l0013					; then jump

_print_a:
	jp		print_sp				; immediate jump

l0013:
	ld		(kcur), hl				; move cursor to position of error
	jr		error_2					; then jump

_get_ch:
	ld		hl, (chadd)				; put address of current character
	ld		a, (hl)					; in A

test_sp:
	call	l007d					; check for printable character
	ret		nc						; return if so

_next_ch:
	call	ch_add_1				; increment current character address
...
error_3:
	ld		(iy + _errnr), l		; then copy to errnr
	ld		sp, (errsp)				; put value of errsp in SP
	jp		set_mem					; then jump
...
nmi:
	...
	jp		(hl)					; jump to address in nmiadd
...
ch_add_1:
	ld		hl, (chadd)				; get current character address

cursor_so:
	inc		hl						; increment it

temp_ptr:
	ld		(chadd), hl				; store it
...
l007d:
	...
	jr		c, l0094				; jump if not
	inc		hl						; else advance another character 

l0094:
	scf								; set carry flag
	ld		(chadd), hl				; put new character address in chadd
	ret								; end of subroutine
...
l0099:
	...
	cp		ctrl_n_l				; test for newline
	ret		z						; return if so
	call	l100e					; advance cursor position
...
; C holds the offset to the routine
	...
	ld		(attrt), sp				; save old stack pointer
...
pr_all_f:
	...
	ld		a, 8					; 
	out		(c), a					; 
	ld		sp, (attrt)				; restore stack pointer
...
.ifdef ROM0
	defs	40, 255					; 40 spare bytes
.endif

.ifdef ROM1
l1396:
	incbin	"copyright.txt"			;
.endif
...
.ifdef ROM0
	...
	jr		c, l223e		; jump with ink
	ld		a, 7			; invert with paper
	sub		d				;
.endif
.ifdef ROM1
	ld		a, d
...
font:
.ifdef ROM0
	incbin	"Chloe.data"			; hi-res 80 column font
.endif

.ifdef ROM1
	incbin	"GenevaMono.data"		; lo-res 40 column font
.endif
